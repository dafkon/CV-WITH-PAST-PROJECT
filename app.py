...
class Buddy(MDApp):
    ...
    def forgotpassword(self, email):
        ...
        try:
            ...
            if kv.current == "ForgotPassword":
                kv.current = "Login"
        # except used to detect when the email address typed in isnt on the system
        except:
    ...
    def sign_up_all_fiedls(self):
        popup = Popup(title='Data not enterd', title_align='center', size_hint=(0.8, 0.3),
                      content=Label(text='Please make sure all \n fields are filled in.', halign='center'))
        popup.open()
    ...
    def signup(self, email, password, password_retype):
        try:
            # Creates a new username and password from input and store it in the firebase
            # auth servers
            if password != password_retype:
                popup = Popup(title='ERROR', title_align='center', size_hint=(0.8, 0.2),
    ...
    def Deletechats(self):
        ...
        box.add_widget(Button(text="Delete User", on_press=lambda instance:
        self.clearchathistory(email_input.text, password_input.text, popup), size_hint=(0.5, 0.5),
    ...
    def clear_chatbot_widgets(self):
        box = BoxLayout(orientation='vertical', padding=20)
        box.add_widget(
            Label(text="Your chat widgets \n have been cleared.", halign="center"))
        popup2 = Popup(title='Chatbot Widgets Cleared', title_size=20, title_align='center', content=box,
                       size_hint=(None, None), size=(200, 200), auto_dismiss=True)
        popup2.open()

    def Showhistroy_popup(self):
        box = BoxLayout(orientation='vertical', padding=20)
        box.add_widget(
            Label(text="Your chat history can \n now be seen on the \n chatbot.", halign="center"))
        popup2 = Popup(title='Chat history Enabled', title_size=20, title_align='center', content=box,
                       size_hint=(None, None), size=(200, 200), auto_dismiss=True)
        popup2.open()
    ...
    def response(self, message):
        ...
        try:
            ...
            response_data = "Sorry there buddy i dont know what you mean by that, if this issue continues please use " \
                            "the menu feature for easy chat communication or try answering the question with a more " \
                            "direct" \
                            "response to help me with me dog like memory pal!! if the question i ask you goes like this" \
                            "'Do you eat healthy?' to respond to this you could type 'Yes i do eat healthy' or " \
                            "'No i dont eat healthy' this way i can like no to eating healthy. So lets try again, 3! 2!" \
                            "1! from the top!!!!"
